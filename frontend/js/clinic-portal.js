...
(function () {
  ...
  let selectedCatId = '';
  let selectedCatName = '';
  let clinicProfile = typeof getOrgProfile === 'function' ? getOrgProfile() : null;
  ...
  function getClinicToken() {
    if (typeof getOrgToken === 'function' && getOrgToken()) return getOrgToken();
    return typeof getToken === 'function' ? getToken() : '';
  }

  function hasClinicSession() {
    clinicProfile = typeof getOrgProfile === 'function' ? getOrgProfile() : clinicProfile;
    if (typeof isOrgLoggedIn === 'function') {
      return isOrgLoggedIn() && !!(clinicProfile && clinicProfile.type === 'clinic');
    }
    return typeof isLoggedIn === 'function' ? isLoggedIn() : false;
  }

  function syncClinicProfile() {
    clinicProfile = typeof getOrgProfile === 'function' ? getOrgProfile() : clinicProfile;
    var orgName = clinicProfile && clinicProfile.name ? clinicProfile.name : 'Clinic Account';
    if (clinicNameEl) clinicNameEl.textContent = orgName;
    if (clinicIdEl) clinicIdEl.textContent = clinicProfile && clinicProfile.id ? ('ID: ' + clinicProfile.id.slice(0, 12) + '...') : 'Not signed in';
    ...
    if (headerLabel) headerLabel.textContent = 'Certified healthcare partner · Official report upload access';
  }
  ...
  async function loadAuthorizedCats() {
    ...
    try {
      var res = await Promise.all([
        fetch(API + '/clinic/cats', { headers: getClinicHeaders() }),
        fetch(API + '/clinic/permissions', { headers: getClinicHeaders() })
      ]);
  ...
  window.selectCat = async function (catId, catName) {
    ...
    try {
      var res = await fetch(API + '/health/records/' + catId, { headers: getClinicHeaders() });
  ...
  function renderOwnerRecords(records) {
    ...
    ownerRecsEl.innerHTML = records.map(function (r) {
      ...
      var endorsementActionHtml = hasEndorsement
  ...
  window.printReport = async function (reportId) {
    try {
      showStatus('正在生成報告…');
      var res = await fetch(API + '/clinic/reports/' + reportId + '/print', { headers: getClinicHeaders() });
      var body = await res.json();
      ...
      if (fileInput && fileInput.files[0]) {
        ...
        try {
          ...
          var uploadRes = await fetch(API + '/health/upload', {
            method: 'POST',
            headers: { 'Authorization': 'Bearer ' + getClinicToken() },
            body: formData
          });
  ...
  if (testApplyBtn) {
    testApplyBtn.addEventListener('click', function () {
      if (!hasClinicSession()) { showStatus('請先登入診所工作人員帳號', true); return; }
      showStatus('載入中…');
      loadAuthorizedCats();
    });
  }
